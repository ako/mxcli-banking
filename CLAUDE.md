...
```bash
./mxcli -p RRNetBanking.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p RRNetBanking.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check RRNetBanking.mpr   # Validate project
./mxcli docker check -p RRNetBanking.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p RRNetBanking.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p RRNetBanking.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p RRNetBanking.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p RRNetBanking.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p RRNetBanking.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'RRNetBanking.mpr';
```
...
./mxcli check script.mdl -p RRNetBanking.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p RRNetBanking.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p RRNetBanking.mpr

# With colored output
./mxcli lint -p RRNetBanking.mpr --color

# List available rules
./mxcli lint -p RRNetBanking.mpr --list-rules

# Output as SARIF
./mxcli lint -p RRNetBanking.mpr --format sarif > results.sarif
```
...
./mxcli report -p RRNetBanking.mpr

# JSON report
./mxcli report -p RRNetBanking.mpr --format json

# HTML report
./mxcli report -p RRNetBanking.mpr --format html
```
